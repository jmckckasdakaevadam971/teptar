...
- **ВАЖНО — недавняя смена курса:** функциональность **«Моё древо»** (личное родовое
  древо, карточки персон, построитель родни, каталог опубликованных древ) была
  **полностью удалена** по решению владельца — и из интерфейса, и из данных.
  См. раздел **«6. Что было удалено»**. Это не баг, так задумано.
- Сейчас сайт = **публичный лендинг + справочник тейпов/тукхумов/сёл + личный кабинет
  и админка/модерация**. Построения деревьев в UI больше нет.
- **Капча (Cloudflare Turnstile) ОТКЛЮЧЕНА** на проде (см. раздел 7). Вход/регистрация
  работают без проверки на бота.
- Бэкенд-эндпоинты деревьев (persons/ancestors/relations) физически ещё существуют в коде,
  но **фронтенд их больше не вызывает**. Их можно оставить или вычистить (см. раздел 10).
...
- модераторам/админам — управлять пользователями и ролями в **админке**.

Роли: `viewer` (читатель) → `editor` → `teip_admin` (админ тейпа) → `super_admin`.
«Модератор» = `teip_admin` или `super_admin`.

> ⚠️ Раздел модерации **общих древ** в UI админки технически присутствует
> (`ModerationPanel`), но реальных древ в базе сейчас нет (данные persons очищены),
> поэтому очередь пуста. Если функциональность древ не вернут — этот компонент
> можно убрать.
...
## 6. ⭐ Что было удалено (история смены курса)
